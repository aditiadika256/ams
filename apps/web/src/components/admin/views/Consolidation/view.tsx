...
import { 
  ...
  RefreshCw, Layers, ShieldCheck, CheckCircle2 
} from 'lucide-react';
  ...
  if (!data) {
    return (
      <div className="grid min-h-64 place-items-center p-8 text-center">
        <div>
          <Building2 className="mx-auto mb-3 size-8 text-zinc-400" />
          <p className="font-medium">Data konsolidasi tidak tersedia</p>
          <Button variant="outline" size="sm" onClick={fetchConsolidation} className="mt-4">
    ...
    >
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3">
          <Button variant="outline" size="sm" onClick={fetchConsolidation} disabled={loading} className="rounded-xl">
            <RefreshCw className={`mr-1.5 size-3.5 ${loading ? 'animate-spin' : ''}`} /> Segarkan Data
          </Button>
        </div>
      </div>
