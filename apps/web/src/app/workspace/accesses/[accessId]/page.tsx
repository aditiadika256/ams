...
import type { CurriculumModule, WorkspaceComponent, WorkspaceSessionUpdate } from '@/types/workspace';
import { ComponentContentRenderer, WorkspaceMediaButton } from '@/components/workspace/ComponentContentRenderer';
...
export default function WorkspaceDetailPage() {
  ...
  const [activeGenericComponent, setActiveGenericComponent] = useState<WorkspaceComponent | null>(null);
  ...
  return (
    <ProtectedRoute>
      <main className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <section className="my-10"><div className="mb-5"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Available tools</p><h2 className="mt-2 text-2xl font-semibold">Komponen Program</h2></div>{access.components?.length ? <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{access.components.map((component) => { const Icon = componentIcons[component.code] ?? FolderOpen; const route = component.handler_template === 'NATIVE' ? componentRoutes[component.code] : undefined; const href = route?.(access.id); const nativeActionable = href && (component.code !== 'meeting' || access.next_session) && (component.code !== 'certificate' || access.certificate); const genericActionable = component.handler_template !== 'NATIVE'; return <Card key={component.id} className="border-zinc-200 shadow-none dark:border-zinc-800"><CardContent className="grid gap-4 p-5"><div className="grid size-11 place-items-center rounded-xl bg-primary/10 text-primary"><Icon className="size-5" aria-hidden="true" /></div><div><h3 className="font-semibold">{component.label || component.name}</h3><p className="mt-1 text-sm text-zinc-500">{genericActionable ? `Isi ${component.handler_template.toLowerCase()} yang diterbitkan untuk Program.` : `Komponen ${component.code} untuk enrollment ini.`}</p></div>{genericActionable ? <Button type="button" variant="outline" onClick={() => setActiveGenericComponent(component)}>Buka isi</Button> : nativeActionable ? <Button asChild variant="outline"><Link href={href}>Buka komponen</Link></Button> : <p className="text-sm text-zinc-500" role="status">Belum ada aktivitas yang dapat dibuka.</p>}</CardContent></Card>; })}</div> : <div className="rounded-2xl border border-dashed p-8 text-center text-sm text-zinc-500">Belum ada komponen yang dapat diakses untuk status enrollment ini.</div>}</section>

        {activeGenericComponent && <ComponentContentRenderer accessId={access.id} component={activeGenericComponent} onClose={() => setActiveGenericComponent(null)} />}
        ...
        {access.components?.some((component) => component.code === 'material') && <section id="material" className="scroll-mt-24"><div className="mb-5"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Curriculum</p><h2 className="mt-2 text-2xl font-semibold">Materi belajar</h2></div>{curriculumLoading ? <div className="h-32 animate-pulse rounded-2xl bg-zinc-100 motion-reduce:animate-none dark:bg-zinc-900" /> : curriculum.length ? <div className="grid gap-4">{curriculum.map((module) => <Card key={module.id} className="border-zinc-200 shadow-none dark:border-zinc-800"><CardHeader><CardTitle className="text-lg">{module.title}</CardTitle>{module.description && <p className="text-sm text-zinc-500">{module.description}</p>}</CardHeader><CardContent className="grid gap-3">{module.lessons.map((lesson) => <article key={lesson.id} className="grid gap-3 rounded-xl bg-zinc-50 px-4 py-4 text-sm dark:bg-zinc-900"><div className="flex min-h-11 flex-wrap items-center gap-3"><BookOpen className="size-4 text-primary" aria-hidden="true" /><span className="min-w-0 flex-1 font-medium">{lesson.title}</span><Badge variant="outline">{lesson.content_kind}</Badge>{access.status === 'ACTIVE' && <Button type="button" variant="outline" size="sm" disabled={completingLesson === lesson.id} onClick={() => void completeLesson(lesson.id)}>{completingLesson === lesson.id ? 'Menyimpan...' : <><CheckCircle2 className="mr-2 size-4" aria-hidden="true" />Tandai selesai</>}</Button>}</div>{(lesson.content_kind === 'INFORMATION' || lesson.content_kind === 'EMBEDDED_PAGE') && lesson.content_body && <div className="whitespace-pre-wrap border-t border-zinc-200 pt-3 leading-7 text-zinc-600 dark:border-zinc-800 dark:text-zinc-300">{lesson.content_body}</div>}{(lesson.content_kind === 'EXTERNAL_LINK' || lesson.content_kind === 'VIDEO') && lesson.external_url && <Button asChild variant="outline" className="justify-self-start"><a href={lesson.external_url} target="_blank" rel="noopener noreferrer">Buka materi<ExternalLink className="ml-2 size-4" /></a></Button>}{lesson.media_asset && <WorkspaceMediaButton asset={lesson.media_asset} label={lesson.content_kind === 'VIDEO' ? 'Buka video' : 'Unduh file'} />}</article>)}</CardContent></Card>)}</div> : <div className="rounded-2xl border border-dashed p-8 text-center text-sm text-zinc-500">Materi terbit belum tersedia.</div>}</section>}
      </main>
    </ProtectedRoute>
  );
}
